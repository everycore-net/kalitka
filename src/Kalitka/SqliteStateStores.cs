...
public sealed class SqliteRequestStore : IRequestStore
{
    ...
    public SqliteRequestStore(string path)
    {
        ...
        foreach (var (col, def) in new[] { ("required_approvals", "INTEGER NOT NULL DEFAULT 1"), ("profile", "TEXT NOT NULL DEFAULT ''"), ("max_uses", "INTEGER NOT NULL DEFAULT 0"), ("command", "TEXT NOT NULL DEFAULT ''"), ("source_addr", "TEXT NOT NULL DEFAULT ''"), ("subject_identity", "TEXT NOT NULL DEFAULT ''") })
        {
    ...
    public void Add(PendingRequest r)
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            INSERT INTO requests(id,target,input,ip,resource,country,country_code,city,raised,state,grant_tok,required_approvals,profile,max_uses,command,source_addr,subject_identity)
            VALUES($id,$target,$input,$ip,$resource,$country,$cc,$city,$raised,$state,$grant,$req,$profile,$uses,$command,$srcaddr,$subjid)
            ON CONFLICT(id) DO UPDATE SET
              ...
              profile=$profile,max_uses=$uses,command=$command,source_addr=$srcaddr,subject_identity=$subjid;
            """;
        Bind(cmd, r);
        cmd.ExecuteNonQuery();
    }
    ...
    private const string Cols =
        "id,target,input,ip,resource,country,country_code,city,raised,state,grant_tok,required_approvals,profile,max_uses,command,source_addr,subject_identity";
    ...
    private static PendingRequest Read(SqliteDataReader r) => new()
    {
        ...
        SubjectIdentity = r.GetString(16)
    };
}
